...
import com.longhan.huang.homeinter.HomeInterHandle;
...
public class LocationService extends Service implements AMapLocationListener {
    ...
    public static final String HomeInterServicePackName = "com.longhan.huang.homeinter.service.LocationService";
    ...
    private TCPConnect TCPCommunication;
    ...
    public class HomeInterBinder extends Binder {
        ...
        public Set<String> getUserList() {
            return LocationService.this.getUserList();
        }
    ...
    public void setUserMetaDatas(String name,String uid, Location location) {
        synchronized (userMetaDatas) {
            ...
            if (TCPCommunication != null && name.equals(getUid())) {
...
//            float bearing = location.getBearing();
                ...
                TCPCommunication.sendMsg(TCPInterface.heartMsg(lon, lat,/* alt, speed, bearing,*/ accuracy));
    ...
    private String getUserNickName(String uid) {
        ...
        String msg = httpConnect.sendMsg(HTTPInterface.getUserNickNameMsg(uid));
        if (msg == null) {
            return nickName;
        }
    ...
    public void onCreate() {
            ...
            mInitUserInfo = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mUid == null || mUid.equals("")) {
                        ...
                        String msg = HTTPConnect.sendMsg(HTTPInterface.connectMsg(deviceID));
                        if (msg == null) {
                            return;
                        }
                        JSONObject jsonObject = null;
        ...
        if (homeInterServiceThread == null) {
            homeInterServiceThread = new Thread("LocationService") {
                @Override
                public void run() {
                    Looper.prepare();
                    try {
                    ...
                    handler = new HomeInterHandle(LocationService.this);
                    ...
                    do {
                        ...
                        TCPCommunication = TCPCommunication.createConnect(handler);
                        if (TCPCommunication == null) {
                            isSocketInitiated = false;
                        } else {
                            handler.setTcpConnect(TCPCommunication);
                            isSocketInitiated = true;
                        }

                    } while (!isSocketInitiated && isBeginService);
                    mServiceStatus = ServerStatus.initConnectAfter;
                    TCPCommunication.sendMsg(TCPInterface.connectMsg(getTCode(), getUid()));
