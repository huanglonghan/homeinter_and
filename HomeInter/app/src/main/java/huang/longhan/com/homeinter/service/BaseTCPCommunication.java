 ...
 */
public class BaseTCPCommunication {
    ...
    private BaseTCPCommunication() {
    }

    public static BaseTCPCommunication createConnect(Handler handler){
        BaseTCPCommunication baseTCPCommunication = new BaseTCPCommunication();
        if(!baseTCPCommunication.initSocket(handler)){
            return null;
        }
        baseTCPCommunication.receiveMsg();
        return baseTCPCommunication;
    }
                        ...
                        if (dataLength == -1) {
                            ...
                            try {
                                while ((dataLength = bufferedInputStream.read(buffer)) != -1) {
                                    if (dataLength == 1024) {
                                        checkLen = BaseTCPProtocol.checkProtocol(buffer, dataLength);
                                        if (checkLen==0){
                                            strBuffer.append(new String(buffer));
                                            Message message = handler.obtainMessage(RECEIVE_DATA_MSG);
                                            Bundle bundle = new Bundle();
                                            bundle.putString(RECEIVE_DATA, BaseTCPProtocol.parseProtocol(strBuffer));
                                        ...
                                        if (strBuffer.length()<1024){
                                            if (BaseTCPProtocol.checkProtocol(buffer, dataLength)!=0){
                                                break;
                                            }
    ...
    public boolean sendMsg(String str) {
        ...
        try {
            bufferedOutputStream.write(BaseTCPProtocol.buildProtocol(str));
